...
plt.title("Logistic Regression (Binary) — Classification Plot")
plt.show()


# ================================================================
# 📌 K-MEANS CLUSTERING — WINE DATASET (with Silhouette Score)
# ================================================================
...
plt.title(f"K-Means Clustering (k={k}) — Wine Dataset")
plt.show()


# ========================================================
# 📌 PCA (Principal Component Analysis) — Wine Dataset
# ========================================================
...
# 6️⃣ Show Original vs Reconstructed Image
...
print("""
Analysis:
- Sigmoid and Tanh saturate for large positive/negative inputs → gradients approach zero (vanishing gradient problem).
- Tanh is zero-centered, which can speed up training compared to Sigmoid.
- ReLU gradient is 1 for positive inputs and 0 for negative inputs → avoids vanishing gradient for positive values.
- ReLU outputs zero for negative inputs → sparsity, but can suffer from "dying ReLU" problem if neurons never activate.
""")


# ================================
# 📌 LAB 5 + 6 VACUUM CLEANER 
# ================================
...
# Objective (convert max → min by multiplying with -1)
c = [-70, -80]
...
plt.xlim(0, 200)
plt.ylim(0, 200)
plt.xlabel("x₁ (Units of Product 1)")
plt.ylabel("x₂ (Units of Product 2)")
...
# ====================================================
# 📌 GA Implementation for the Rastrigin Function
# ====================================================
...
# --- 1. Rastrigin Function Definition (CORRECTED) ---

def rastrigin(X):
    """
    The 2D Rastrigin function for optimization.
    X is an individual (array-like, e.g., [x, y]).
    
    CORRECTION: Explicitly convert input to a NumPy array (np.asarray(X)) 
    to prevent the 'TypeError: unsupported operand type(s) for ** or pow(): 'list' and 'int''
    when generating the heatmap mesh.
    """
    X = np.asarray(X) # Ensure input is a NumPy array for element-wise operations
    A = 10
    n = len(X)
    # The function to minimize is F(x) = A*n + sum(xi^2 - A*cos(2*pi*xi))
    return A * n + np.sum(X**2 - A * np.cos(2 * np.pi * X))

# --- 2. GA Parameters ---

# Population size (number of individuals/solutions)
POP_SIZE = 50
# Number of generations (iterations)
MAX_GENERATIONS = 100
# Bounds for the search space (e.g., typically [-5.12, 5.12])
BOUNDS = (-5.12, 5.12)
...
def initialize_population(pop_size, bounds):
    """Initializes a random population within the given bounds[cite: 19]."""
    # Each individual is [x, y]
    lower_bound, upper_bound = bounds
    return np.random.uniform(lower_bound, upper_bound, (pop_size, 2))

def evaluate_fitness(population):
    """Calculates the fitness (Rastrigin function value) for each individual[cite: 20]."""
    return np.array([rastrigin(individual) for individual in population])

def selection(population, fitness):
    """
    Selects parents using tournament selection[cite: 22].
    Lower Rastrigin value (closer to 0) is better fitness.
    """
    tournament_size = 5
    parents = []
    
    # We need POP_SIZE parents for the next generation's mating pool
    for _ in range(POP_SIZE):
        # Randomly select individuals for the tournament
        indices = np.random.choice(POP_SIZE, tournament_size, replace=False)
        tournament_individuals = population[indices]
        tournament_fitness = fitness[indices]
        
        # Find the index of the best (minimum fitness) in the tournament
        best_index_in_tournament = np.argmin(tournament_fitness)
        
        # Select the best individual as a parent
        parents.append(tournament_individuals[best_index_in_tournament])
        
    return np.array(parents)

def crossover(parents, c_rate):
    """
    Applies arithmetic crossover to create offspring[cite: 23].
    """
    offspring = np.empty_like(parents)
    
    # Iterate over parents in pairs
    for i in range(0, POP_SIZE, 2):
        parent1 = parents[i]
        # Handle odd population size by pairing the last individual with the first
...
def mutation(offspring, m_rate, bounds):
    """
    Applies Gaussian/creep mutation to the offspring[cite: 24].
    """
    lower_bound, upper_bound = bounds
    
    for individual in offspring:
        # Check if mutation should occur for this individual
        if np.random.rand() < m_rate:
            # Mutate one of the two dimensions (x or y)
            dimension_to_mutate = np.random.randint(2)
            
            # Gaussian mutation: add a small random value (sigma=0.5)
            individual[dimension_to_mutate] += np.random.normal(0, 0.5)
            
            # Clamp the values back to the search bounds
            individual[dimension_to_mutate] = np.clip(individual[dimension_to_mutate], lower_bound, upper_bound)
            
    return offspring

def replace_population(old_population, old_fitness, new_offspring, elitism):
    """Forms the new population, retaining the best individual (elitism) if enabled[cite: 26]."""
    
    new_population = new_offspring
    
    if elitism:
        # Find the best individual (minimum fitness) in the old population
        best_old_index = np.argmin(old_fitness)
        best_old_individual = old_population[best_old_index]
        best_old_fitness = old_fitness[best_old_index]
        
        # Evaluate fitness of the new population [cite: 25]
        new_fitness = evaluate_fitness(new_offspring)
        
        # Find the worst individual (maximum fitness) in the new population
        worst_new_index = np.argmax(new_fitness)
        worst_new_fitness = new_fitness[worst_new_index]
        
        # If the best old individual is better than the worst new one, replace the worst new.
...
def genetic_algorithm(pop_size, max_generations, bounds, c_rate, m_rate, elitism):
    """Main function to run the GA optimization."""
    
    # 0. Start & 1. Initialize population
    ...
    for gen in range(max_generations):
        ...
        # Store data for visualization/analysis
        history.append({
            'generation': gen,
        ...
        # 21. Repeat until max generations:
        # 22. Select parents
        parents = selection(population, fitness)
        
        # 23. Apply crossover
        offspring = crossover(parents, c_rate)
        
        # 24. Mutate offspring
        mutated_offspring = mutation(offspring, m_rate, bounds)
        
        # 25 & 26. Form new population (with elitism)
        population = replace_population(population, fitness, mutated_offspring, elitism)
        
        if gen % 10 == 0:
            print(f"Generation {gen}/{max_generations}: Best Fitness = {best_fitness:.4f}, Solution = {best_individual}")

    # 27. Return best solution found
    final_best_index = np.argmin(evaluate_fitness(population))
    final_best_solution = population[final_best_index]
    final_best_fitness = rastrigin(final_best_solution)

    print("\nGA finished[cite: 28].")
    print(f"Final Best Solution (x*, y*): {final_best_solution}")
    print(f"Minimum Rastrigin Value: {final_best_fitness:.6f}")
    
    return final_best_solution, final_best_fitness, history

# --- 5. Visualization ---

def visualize_results(history, bounds, generations_to_plot=None):
    """
    Produces heatmap visualizations of the Rastrigin landscape 
    and overlays GA population points to show search dynamics[cite: 9, 33].
    """
    ...
    # Calculate Z (fitness) for the entire grid
    Z = np.array([rastrigin([x, y]) for x, y in zip(np.ravel(X), np.ravel(Y))]).reshape(X.shape)

    if generations_to_plot is None:
        # Plot start, mid-convergence, and end
        generations_to_plot = [0, 10, 50, len(history) - 1] 

    fig, axes = plt.subplots(1, len(generations_to_plot), figsize=(18, 6), sharex=True, sharey=True)
    
    # Ensure axes is iterable even for a single plot
    if len(generations_to_plot) == 1:
        axes = [axes]
        
    fig.suptitle('Genetic Algorithm Search Dynamics on Rastrigin Function (Heatmap Visualization)', fontsize=16)

    for i, gen_idx in enumerate(generations_to_plot):
        data = history[gen_idx]
        population = data['population']
        
        ax = axes[i]
        
        # Heatmap of the Rastrigin function
        c = ax.contourf(X, Y, Z, levels=20, cmap=cm.viridis)
        # Adding a colorbar to the first plot is often sufficient
        if i == 0:
             fig.colorbar(c, ax=ax, shrink=0.7) 
        
        # Overlay GA population points
        ax.scatter(population[:, 0], population[:, 1], c='red', marker='o', s=15, alpha=0.6, label='Population')
        
        # Highlight the best point
        best_solution = data['best_solution']
        ax.scatter(best_solution[0], best_solution[1], c='yellow', marker='*', s=200, edgecolors='black', label='Best Solution')
        
        ax.set_title(f'Generation {data["generation"]}\nBest Fitness: {data["best_fitness"]:.4f}')
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_xlim(bounds)
        ax.set_ylim(bounds)
...
# --- Run the GA and Visualize ---

if __name__ == "__main__":
    # --- Code Execution [cite: 29] ---
    final_solution, final_fitness, ga_history = genetic_algorithm(
        pop_size=POP_SIZE, 
    ...
    # Visualize the search over key generations
    visualize_results(ga_history, BOUNDS)
    # --- End of Execution [cite: 30] ---